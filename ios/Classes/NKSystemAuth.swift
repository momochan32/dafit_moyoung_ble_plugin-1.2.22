...
//            alertV.show()
            ...
            NKSimpleAlertController.presentAlertVc(confirmBtn: "确定", message: "相机权限受限", title: "提示", cancelBtn: "", handler: { action in
...
//             print("无法解析出JSONString")
    ...
    /// 将基本类型转为json
    /// - Parameter data: 基本类型的数据
    /// - Returns: json
    class func getJsonStringFromBasicTypeData(data: Any) -> String {
    ...
    /// - Returns: Date
    class func timeToDate(timeStamp : Int) -> Date {
    ...
    /// 获取年、月、日
    /// - Returns: 月
    class func getDateComponent(type: Calendar.Component, date: Date) -> Int {
        let calendar = Calendar.current
        let month = calendar.component(type, from: date)
        return month
    }
    
    /// 获取时间戳
    /// - Returns: 时间戳
    public static func getTimeInterval(date:Date) -> Int{
    ...
    /// - Parameter time: 时间
    /// - Returns: Date
    class func timeToStamp(time : String,_ type: String = "yyyy-MM-dd") -> Date {
    ...
    /// 时间字符串转分钟数
    /// - Parameter timeString: 小时:分钟
    /// - Returns: 分钟数
    public class func totalMinutes(from timeString: String) -> Int? {
        let components = timeString.components(separatedBy: ":")
        guard components.count == 2,
              let hours = Int(components[0]),
              let minutes = Int(components[1]) else {
            return nil // 返回 nil 表示转换失败
        }
